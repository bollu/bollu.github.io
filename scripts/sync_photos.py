#!/usr/bin/env python3
"""Sync the public Nextcloud photo album into photos-sync/.

Lists the album via public WebDAV, then downloads a 2048px preview of every
photo (skipping ones already present, pruning ones that left the album).
photos-sync/ is a gitignored staging area: run downscale_photos.py after
this to refresh the committed web-size copies in static/photos/.
"""
...
OUT = os.path.join(os.path.dirname(__file__), "..", "photos-sync")
...
def main():
    ...
    print(f"fetched {fetched} new previews; staging has "
          f"{len([f for f in os.listdir(OUT) if f.endswith('.jpg')])} photos")
