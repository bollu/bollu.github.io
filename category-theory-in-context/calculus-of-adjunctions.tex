...
Stare at the large diagram:


% https://q.uiver.app/?q=
\[\begin{tikzcd}
	F &&& F \\
	F & FGF & F \\
	{F GF} & FGFGF & FGF \\
	...
	\arrow["{\epsilon'_F}", from=2-2, to=2-3]
	\arrow["{F \eta}", from=2-3, to=3-3]
	\arrow["{\epsilon'_F}", from=3-3, to=4-3]
	\arrow["{\epsilon' F \circ F \eta}", from=1-1, to=1-4]
	\arrow["{\epsilon'F \circ F \eta}", from=1-4, to=4-4]
	\arrow["{F \eta}"', from=2-1, to=3-1]
	\arrow[no head, from=3-1, to=4-2]
	\arrow["{\epsilon'_F}"', from=4-2, to=4-3]
	\arrow[shift right=1, no head, from=3-1, to=4-2]
	\arrow["{F\eta_{GF}}", from=3-1, to=3-2]
	\arrow["{FG\epsilon'_F}", from=3-2, to=4-2]
	\arrow["FGF\eta", from=2-2, to=3-2]
	\arrow["{\epsilon'_{FGF}}", from=3-2, to=3-3]
\end{tikzcd}\]

\begin{itemize}
    \item Throughout this diagram, we use the equivalence between $\alpha K$
      and $\alpha_K$ for functor $K: X \to Y$ and natural transformation
      $\alpha: L \Rightarrow M$ for $L, M: Y \to Z$ (this makes $\alpha K / \alpha_K: X \to Z$.
    \item The top and right witness $(\epsilon' F \circ F \eta)^2$. The bottom witnesses $(\epsilon' F \circ F \eta)$. The commutativity of the whole
      square witnesses idempotence.
    \item The top left square commutes due to the naturality of $F \eta$:

% https://q.uiver.app/?q=
\[\begin{tikzcd}
	& {F(1_F) \simeq F} & {F(GF)} \\
	x & {F(1_Fx)} & {F(GFx)} \\
	{\eta_x} & {F ( GF x)} & {F(GFGFx)}
	\arrow["{F\eta_x}", from=2-2, to=2-3]
	\arrow["{F \eta}"', from=2-2, to=3-2]
	\arrow["{F\eta_{GFx}}", from=3-2, to=3-3]
	\arrow["FGF\eta", from=2-3, to=3-3]
	\arrow[from=2-1, to=3-1]
	\arrow["F\eta"', from=1-2, to=1-3]
\end{tikzcd}\]

\item For basically the exact same reasons, the top-right square commutes due to the naturality of $\epsilon'F$ (which is equal to $\epsilon'_F$.
\item The bottom-right square commutes due to the naturality of $\epsilon'F$.
\item Now we're left with showing that the bottom-left triangle commutes. See that it asserts that $FG\epsilon'_F \circ F \eta_{GF} = 1_{FGF}$.
  Refactoring the equation, we can write this as $F (G \epsilon' \circ \eta G) F = F (1_G) F$. This is true by our \emph{previous} proof, where
  we showed that the first triangle identity is obeyed!
\item Since every sub-square in our diagram commutes, the whole diagram commutes, and therefore we have shown the idempotence of $\epsilon' F \circ F \eta$,
    which implies it's equal to the identity.
\end{itemize}

\subsection{Yoneda based proof}

\begin{itemize}
  \item If $\eta_c: 1_C \simeq GF$ is one of the natural isos of an equivalence of categories $F: C \leftrightarrow D: G$, then we define
    the function:
    $$
    \begin{matrix}
      & D(Fc, d) &\xrightarrow{G} &C(GFc, Gd) &\xrightarrow{ - \circ \eta_c} &C(c, Gd) \\
      & f: Fc \to d &\overset{G}{\mapsto}  &Gf: GFc \to Gd &\overset{- \circ \eta_c}{\mapsto} &(Gf: GFc \to Gd) \circ (\eta_c: c \to GFc) : c \to G d
    \end{matrix}
    $$
  \item This is full and faithful : $G$ is full and faithful since it's part of
    an equivalence of categories, and $- \circ \eta_c$ is full and faithful
...
\section{4.4.6: Adjunction raises to adjunction of diagrams}

\begin{itemize}
  \item Suppose $F \vdash G$ where $F: C \to D: G$.
  \item Then we claim that there exists an adjunction between $(J \to C)$ and $(J \to D)$, given by:
  % https://q.uiver.app/?q=WzAsNCxbMCwyLCIoSiBcXHRvIEMpIl0sWzEsMiwiKEogXFx0byBEKSJdLFswLDAsIkMiXSxbMSwwLCJEIl0sWzAsMSwiRiBcXGNpcmMgLSIsMCx7ImN1cnZlIjotMn1dLFsxLDAsIkcgXFxjaXJjIC0iLDAseyJjdXJ2ZSI6LTJ9XSxbMiwzLCJGIiwwLHsiY3VydmUiOi0xfV0sWzMsMiwiRyIsMCx7ImN1cnZlIjotMX1dXQ==
\[\begin{tikzcd}
	C & D \\
	\\
	{(J \to C)} & {(J \to D)}
	\arrow["{F \circ -}", curve={height=-12pt}, from=3-1, to=3-2]
	\arrow["{G \circ -}", curve={height=-12pt}, from=3-2, to=3-1]
	\arrow["F", curve={height=-6pt}, from=1-1, to=1-2]
	\arrow["G", curve={height=-6pt}, from=1-2, to=1-1]
\end{tikzcd}\]
\end{itemize}

\subsection{Yoneda based proof (Exercise 4.4.iii)}

\begin{itemize}
    \item What does it mean to have such an adjunction?
    \item It means that we have a natural identification of hom-sets $Hom_{J \to D}(F_* d_C ,d_D)$
        (where $d_K: J \to K$ is for digram in category $K$ indexed by $J$) with the hom set $Hom_{J \to C}(d_C, G_* d_D)$.
    \item Consider any $\alpha \in Hom_{J \to D}(F_* d_C, d_D)$. We must build a $\alpha^\sharp \in Hom_{J \to C}(d_C, G_* d_D)$ that is natural in $d_C, d_D$.
    \item  What is the data involved in $\alpha^\sharp$? Well, it's a commutative square:
      % https://q.uiver.app/?q=WzAsNyxbMiwwLCJkX0MgaiJdLFsyLDEsImRfQyBrIl0sWzAsMCwiaiJdLFswLDEsImsiXSxbNCwwLCJHZF9EaiJdLFs0LDEsIkcgZF9EayJdLFszLDFdLFswLDEsImRfQ2EiLDJdLFsyLDMsImEiXSxbMCw0LCJcXGFscGhhXlxcc2hhcnBfaiIsMl0sWzEsNSwiXFxhbHBoYV5cXHNoYXJwX3trfSIsMl0sWzQsNSwiR2RfRGEiXV0=
\[\begin{tikzcd}
	j && {d_C j} && {Gd_Dj} \\
	k && {d_C k} & {} & {G d_Dk}
	\arrow["{d_Ca}"', from=1-3, to=2-3]
	\arrow["a", from=1-1, to=2-1]
	\arrow["{\alpha^\sharp_j}"', from=1-3, to=1-5]
	\arrow["{\alpha^\sharp_{k}}"', from=2-3, to=2-5]
	\arrow["{Gd_Da}", from=1-5, to=2-5]
\end{tikzcd}\]
  \item But by the adjointness of $F \vdash G$, the above square commutes iff the square below commutes:

% https://q.uiver.app/?q=
\[\begin{tikzcd}
	j & {Fd_C j} && {d_Dj} \\
	k & {Fd_C k} & {} & {d_Dk}
	\arrow["{Fd_Ca}"', from=1-2, to=2-2]
	\arrow["a", from=1-1, to=2-1]
	\arrow["{(\alpha^\sharp_j)^\flat}"', from=1-2, to=1-4]
	\arrow["{(\alpha^\sharp_{k})^\flat}"', from=2-2, to=2-4]
	\arrow["{d_Da}", from=1-4, to=2-4]
\end{tikzcd}\]
\item We can choose $(\alpha^\sharp)^\flat_j \equiv \alpha_j$, since $\alpha$ witneses the commutativity of exactly this diagram!
\item This means that the map which links the Hom-sets is the transpose map, which transposes a natural transformation pointwise: 
    $(\alpha^\sharp)_x \equiv (\alpha_x)^\sharp$.
\end{itemize}

\subsection{unit-counit based proof (Exercise 4.4.ii}

\begin{itemize}
  \item Denote as $F_*: (J \to C) \to (J \to D)$. (Notational note: you fall
    forward and down toward the star, so pushforward is lower star; you look up
    and fall back to look at the stars, so pull back is upper star). This is a push forward
    since it's like pushing forward a curve $[0, 1] \to C$ via a map $F: C \to D$.
  \item Similarly, denote as $G_*: (J \to D) \to (J \to C)$.
  \item We must create a natural transformation $\overline \eta: 1_{C^J} \to G_* F_* 1_{C^J}$
  \item This means that to any diagram $D: C^J$, we must associate a natural transformation $\overline \eta_D: D \to GFD$.
  \item This can be written componentwise as $(\overline \eta_D)_j: Dj \to GFDj$.
  \item This can be written more clearly as $(\overline \eta_D)_j: Dj \to GFDj$.
  \item This can be given by the component of $\eta: 1_C \to GF$ along $Dj$, which is $\eta_{Dj}: Dj \to GF(Dj)$.
  \item So we define $(\overline \eta_D)_j \equiv \eta_{Dj}$.
\end{itemize}
