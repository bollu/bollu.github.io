...
this a \mathbf{representation of the Lie group}, since $\mathfrak g$ is
a vector space!
...
The collection of maps $Ad_g^\star$ forms the _co-adjoint_ representation of $G$
...
on $M$. I am confused because $X$ is _already_ a left-invariant vector field!
...
$$
i_{\xi_X} \omega = d H_X \quad \text{H: M \rightarrow \mathbb R, \forall X \in \mathfrak g}
$$

we can hope that $H$ is _linear_. that is, $H_{X + Y} = H_X + H_Y, \forall X, Y \in \mathfrak g$.
...
Let $X^# = \xi_X$ be the vector field on $M$ generated by $\{ exp(tX) : t \in \mathbb R} \subseteq G$.

1. Then, $d \mu^X = i_{X^\hash} \omega$ [Condition 1]
